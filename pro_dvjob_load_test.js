 ...
 *   k6 run --out json=results.json dev-dvjob_load_test.js
 *
...
const REGION   = __ENV.REGION   || 'kz';
const LANG     = __ENV.LANG     || 'ru';
...
export const options = {
  scenarios: {
    ...
    //
    // ВНИМАНИЕ: пик снижен с 50 → 20 rps и VUs с 250 → 100, чтобы не
    // спровоцировать rate-limit/WAF на dev-стенде. В прошлом прогоне
    // с target=50 rps стенд уходил в деградацию (~18s на все запросы).
    health_spike: {
...
export function resumeSearchScenario() {
  group('Resume Search', () => {
    ...
    // Тикет DVJOB-0192. После фикса — раскомментировать work_formats ниже.
...
export function setup() {
  console.log('=== dvjob.kz Load Test ===');
  ...
  // Этот эндпоинт мы мерили руками: на здоровом dev — ~0.6s, prod — ~0.9s.
  // Если он отвечает > 2s — стенд в состоянии деградации, полный прогон
  // даст нечитаемые метрики (все запросы «упрутся» в общий latency).
  ...
  // 3. Контрольная проба известного бага resumes/filter + work_formats
